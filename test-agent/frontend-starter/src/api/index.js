...
export const savePreset = async (agentId, name) => {
  const response = await api.post(`/api/agents/${agentId}/presets`, { name });
  return response.data;
};

export const deletePreset = async (agentId, name) => {
  const response = await api.delete(`/api/agents/${agentId}/presets/${encodeURIComponent(name)}`);
  return response.data;
};
...
export const renderTemplate = async (id, values) => {
  const response = await api.post(`/api/templates/${id}/render`, { values });
  return response.data;
};
