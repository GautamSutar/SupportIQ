// mockLlmApi.js
// LLM API for the challenge, backed by OpenRouter (chat) and Pinecone (retrieval).
// Falls back to local simulation if the live API call fails.

const express = require('express');
const router = express.Router();
const axios = require('axios');
const { queryKnowledgeBase } = require('./vectorDbSetup');
...
router.post('/generate', async (req, res) => {
  ...
  try {
    const { conversationId, messages = [], parameters = {}, capabilities, knowledgeBases, agentId } = req.body;
    ...
    const persona = agentPersonas[agentId] || agentPersonas['agent-cs-1'];

    let response;
    let usedFallback = false;
    try {
      const context = knowledgeBases?.length
        ? await queryKnowledgeBase(lastCustomerMessage?.content || '', knowledgeBases, 3)
        : [];

      const systemPrompt = [
        `You are ${persona.name}, a ${persona.style} customer service agent for RetailPlus.`,
        `Knowledge areas: ${persona.knowledge.join(', ')}.`,
        context.length ? `Relevant knowledge base excerpts:\n${context.map(c => `- ${c.text}`).join('\n')}` : '',
        'Respond directly to the customer in 1-3 sentences.',
      ].filter(Boolean).join('\n\n');
      ...
      response = await callOpenRouter(chatMessages, parameters.temperature);
      if (!response) throw new Error('Empty response from OpenRouter');
    } catch (llmError) {
      console.error('OpenRouter call failed, falling back to simulated response:', llmError.message);
      usedFallback = true;
    ...
    const metrics = {
      ...
      confidenceScore: usedFallback
        ? Math.max(0.4, 1 - (parameters.temperature ?? 0.5))
  ...
  try {
    ...
    try {
      const raw = await callOpenRouter([
        {
          role: 'system',
          content: 'Analyze the sentiment of the customer message. Reply with ONLY compact JSON: {"score": <0-1, 0=very negative 1=very positive>, "emotion": "<one word>", "intensity": "<low|medium|high>", "keywords": ["up to 3 words"]}',
        },
        { role: 'user', content: text },
      ], 0.2);

      if (!raw) throw new Error('Empty sentiment response');
      sentiment = JSON.parse(raw.replace(/```json|```/g, '').trim());
    } catch (llmError) {
      console.error('OpenRouter sentiment call failed, falling back to local analysis:', llmError.message);
...
router.post('/knowledge', async (req, res) => {
  try {
    ...
    let results;
    try {
      results = await queryKnowledgeBase(query, knowledgeBases, limit);
    } catch (dbError) {
      console.error('Pinecone query failed, falling back to local knowledge lookup:', dbError.message);
