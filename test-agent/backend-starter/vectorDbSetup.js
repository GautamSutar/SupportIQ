// vectorDbSetup.js - Setup script for the Pinecone vector database

// Loaded here too (not just in server.js) because this module reads process.env
// at require-time to construct the Pinecone client, so it must not depend on
// require order elsewhere. dotenv.config() is a no-op if vars are already set.
...
// Index configuration
const INDEX_NAME = process.env.PINECONE_INDEX_NAME || 'zangoh-knowledge-base';
...
async function setupVectorDb() {
  ...
  try {
    ...
    // Optionally clear existing vectors before re-indexing
    if (process.env.RECREATE_COLLECTION === 'true') {
      console.log(`Clearing index '${INDEX_NAME}'...`);
...
// Create the index if it doesn't already exist, and return a handle to it
...
async function indexKnowledgeBase() {
  ...
  for (const file of files) {
    ...
    const points = chunks.map((chunk, index) => ({
      id: `${kbId}-${index}`,
      values: generateMockEmbedding(chunk.text),
      metadata: {
        text: chunk.text,
    ...
    const index_ = await getIndex();
    const batchSize = 100;
    for (let i = 0; i < points.length; i += batchSize) {
      const batch = points.slice(i, i + batchSize);
      await index_.upsert(batch);
...
// Query the knowledge base for chunks relevant to a piece of text
async function queryKnowledgeBase(queryText, knowledgeBases = [], limit = 3) {
  const index_ = await getIndex();
  const vector = generateMockEmbedding(queryText);

  const filter = knowledgeBases && knowledgeBases.length > 0
    ? { knowledge_base_id: { $in: knowledgeBases } }
    : undefined;

  const result = await index_.query({
    vector,
    topK: limit,
  ...
  return (result.matches || []).map(match => ({
    ...
    source: match.metadata?.knowledge_base_id,
    section: match.metadata?.section_path,
    knowledgeBase: match.metadata?.knowledge_base_id,
    relevance: Number((match.score ?? 0).toFixed(2)),
  }));
}
...
module.exports = { setupVectorDb, queryKnowledgeBase };
